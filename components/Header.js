    ...
    <div className="absolute inset-x-0 z-50  opacity-70 bg-transparent w-full">
      <header className="  inset-x-0 top-0  z-50 max-w-7xl mx-auto ">
        <nav className="flex items-center  justify-between p-6 lg:px-8" aria-label="Global">
          ...
          <div className="flex lg:hidden">
            ...
            >
              ...
              <Bars3Icon className="h-6 w-6 text-[#A4BE7B]" aria-hidden="true" />
            </button>
          </div>
          <div className="hidden lg:flex lg:gap-x-12">
            {navigation.map((item) => (
              <div key={item.name} onClick={()=>scrollToDiv(item.href)} 
              className="text-sm font-semibold  cursor-pointer leading-6 text-white">
                {item.name}
              </div>
            ))}
          </div>
        ...
        <Dialog as="div" className="lg:hidden" open={mobileMenuOpen} onClose={setMobileMenuOpen}>
          ...
          <Dialog.Panel className="fixed inset-y-0 bg-[#285430] right-0 z-50 w-full overflow-y-auto px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
            <div className="flex items-center justify-between">
              ...
              >
                ...
                <XMarkIcon className="h-6 w-6 text-[#A4BE7B]" aria-hidden="true" />
              </button>
            </div>
                ...
                <div className="space-y-2 py-6">
                  {navigation.map((item) => (
                    <div
                      ...
                      className="-mx-3 block  rounded-lg px-3 py-2 text-base font-semibold leading-7 text-[#A4BE7B] hover:bg-gray-50"
                    >
                      {item.name}
                    </div>
                  ))}
